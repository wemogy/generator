...
class SimpleGenerator extends Generator {
  ...
  constructor(args: any, options: any) {
    super(args, options);
    this.log(yosay(`Welcome to the ${chalk.blue(`wemogy`)}!`));

    this.templates = [new ReactAppGenerator(args, options), new ReactLibGenerator(args, options)];
  }
  ...
  public writing(): void {
    ...
    this.fs.copyTpl(`${sourceRoot}templates/${this.selectedTemplate.templateFolderName}`, outputDirectory, {
      title: 'xy',
      ...this.answers
    });
  }
